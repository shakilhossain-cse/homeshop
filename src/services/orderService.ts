...
export const getAllOrders = (
  ...
  HttpClient.get(`all-order/${status}`, { params: { page } }).then(
    (res) => res.data
  );
...
export const getAuthUserOrder = (status: string): Promise<IOrder[]> =>
  HttpClient.get(`/my-order/${status}`).then((res) => res.data);
